...
caldir = '/scratch11/ktp9/DIA/70/master/voltron/' # directory to output the flux files and location of master frame
#clndir = '/scratch11/ktp9/DIA/70/clean70/'#directory with the cleaned images
##END UPDATE####
...
for ii in range(0, len(x)):
	...
	if (x[ii] > 0) and (x[ii] < axs) and (y[ii] > 0) and (y[ii] < axs):
...
for ii in range(0, len(x_pix)):
	if (x_pix[ii] > 0) and (x_pix[ii] < axs) and (y_pix[ii] > 0) and (y_pix[ii] < axs) and (numpy.isnan(mag[ii]) != 1):
		output.write(str(x_pix[ii])+','+str(y_pix[ii])+','+str(mag[ii])+','+str(err[ii])+'\n')
output.close()
...
for ii in range(0, len(x_pix)):
	if (x_pix[ii] > 0) and (x_pix[ii] < axs) and (y_pix[ii] > 0) and (y_pix[ii] < axs) and (numpy.isnan(mag[ii]) != 1):
		output.write(str(x_pix[ii])+','+str(y_pix[ii])+','+str(flx[ii])+','+str(flx_er[ii])+'\n')
output.close()
...
for ii in range(0, len(x_pix)):
	if (x_pix[ii] > 0) and (x_pix[ii] < axs) and (y_pix[ii] > 0) and (y_pix[ii] < axs) and (numpy.isnan(mag[ii]) != 1):
		output.write(str(int(x_pix[ii]))+','+str(int(y_pix[ii]))+'\n')
output.close()

print('All done! Bye-bye, buttrefly!')
