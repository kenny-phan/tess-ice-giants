...
compdiff = os.system('gcc /home/ktp9/TESSNeptune24/code/bigdiff/oisdifference.c -L/home/ktp9/TESSNeptune24/code/bigdiff/cfitsio-4.4.1 -I/home/ktp9/TESSNeptune24/code/bigdiff/cfitsio-4.4.1  -lcfitsio -lm -lcurl')

#useful directories
cdedir = '/home/ktp9/TESSNeptune24/code/bigdiff/' #code directory
caldir = '/scratch11/ktp9/DIA/70/master/voltron/' # directory for the location of the master frame
clndir = '/home/ktp9/TESSNeptune24/code/bigdiff/testclean/' #'/scratch11/ktp9/DIA/70/clean70/'# directory of where the images are located
difdir = '/home/ktp9/TESSNeptune24/code/bigdiff/testdif/' # directory to put the differenced images
...
mast, mheader = fits.getdata(caldir+sector+'_'+camera+'_'+ccd+'_master_py.fits', header = True)
mean, median, std = sigma_clipped_stats(mast, sigma = 3.0, maxiters = 5)
expm_time = fits.getval(caldir+sector+'_'+camera+'_'+ccd+'_master_py.fits','EXPTIME')
...
xx, yy = numpy.loadtxt(caldir+camera+'_'+ccd+'_starlist_py.txt', unpack = 1, delimiter = ',')
xm, ym, mflx, mflx_er = numpy.loadtxt(caldir+camera+'_'+ccd+'_master_py.flux', unpack = 1, usecols = (0,1,2,3), delimiter =',')

#begin with the algorithm to difference the images
for ii in range(0, nfiles):
	hld = files[ii].split('.')
	finnme = hld[0]+'_dxx.'+hld[1]
	...
	if (os.path.isfile(difdir+finnme) == 0):
		...
		while (itr < len(x)): ##messed stuff up here but i hope it still works! <3
		...
		mvdiff = os.system('mv img.fits '+difdir+finnme)
		...
		#get the star flux and error & mag and error
		flx = rawflux['aperture_sum']-bkg_sum
		print(f"fr[ap_sum] = {rawflux['aperture_sum']}")
		print(f"bkg_sum = {bkg_sum}")
		print(f"star flux = {flx}")
		print(f"exp_time = {exp_time}")
		print(f"mflx = {mflx}")
		print(len(mflx))
		print(f"expm_time = {expm_time}")
		flx_er = numpy.sqrt(numpy.abs(rawflux['aperture_sum']))
		...
		for jj in range(0, len(xx)):
			output.write(str(xm[jj])+','+str(ym[jj])+','+str(jd)+','+str(mag[jj])+','+str(mag_er[jj])+'\n')
		output.close()
		print('Moving to the next file at '+strftime("%a, %d %b %Y %H:%M:%S")+'.')

print('All done!  Toodle-loo, kagnaroo!')
