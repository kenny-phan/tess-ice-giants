...
def plot_lat_solutions(axs, latitudes, std, wind_eqn, sector, color, label=False, even=True):
    for i, latitude in enumerate(latitudes):
        ...
        axs.errorbar(wind_eqn(np.radians(latitude)), latitude, yerr=std[i], fmt='o', color=color, capsize=3, label=label)

        if even:
            axs.errorbar(wind_eqn(np.radians(latitude)), -latitude, yerr=std[i], fmt='o', color=color, capsize=3)

def plot_neptune_equations(ax, phi, colors=["xkcd:pale yellow", "xkcd:light orange", "xkcd:pumpkin", "xkcd:purplish pink"], h_band=False):
    ...
    ax.plot(sromovsky1993_four(phi), phi * 180 / np.pi, label="Voyager 4th Order", color=colors[0])
    ax.plot(sromovsky1993_six(phi), phi * 180 / np.pi, label="Voyager 6th Order", color=colors[1])

    ax.plot(tollefson2013_kp(phi), phi * 180 / np.pi, label="Keck K' 2013", color=colors[2])
    ax.plot(tollefson2014_kp(phi), phi * 180 / np.pi, label="Keck K' 2014", color=colors[3])
...
def plot_mosaic_latitudes(mosaic_data, eqns, lats, stds, plot_colors, ibm, title, sector):
    ...
    def select_bright_points(mosaic_data, axs, color, bins=60, percentile=95, plot=True):
        threshold = np.percentile(mosaic_data, percentile)
        bright_points = np.argwhere(mosaic_data >= threshold)
        if plot:
            axs.hist(bright_points[:, 0], bins=bins, orientation="horizontal", density=True, label=f"{100 - percentile}%", alpha=0.5, color=color)
    ...
    for i, percentile in enumerate([70, 80, 90, 95]):
        select_bright_points(mosaic_data, histax, color=ibm[-i], bins=60, percentile=percentile, plot=True)
